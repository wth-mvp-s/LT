...
`FROM
`    OPENROWSET (BULK ‘https: //contosol.dfs.core.windows.net/applications/year=*/*.parquet’,
`FORMAT='PARQUET) AS [r]
`GO
...
`WITH
`   ( LOCATION = ‘https://accountl.blob.core.windows.net',
`    TYPE = HADOOP
`)
...
`CREATE EXTERNAL DATA SOURCE source1
`WITH
`   ( LOCATION = ‘https://accountl.dfs.core.windows.net',
`    TYPE = HADOOP
`)
...
`CREATE EXTERNAL DATA SOURCE source1
`WITH
`   ( LOCATION = ‘https://accountl.table.core.windows.net',
`    PUSHDOWN = ON
`)
...
`::
`Apache Parquet - columnar storage file format that supports schema evolution. 
`    includes metadata 
`
`Support querying a subset of columns in the files:
`    makes it easy to read and process only the necessary columns for a given query.
`
`Support read-heavy analytical workloads: 
...
`
`       other options: Apache Avro:
`
`                        Apache Avro is a binary data serialization format that supports schema evolution. It can store data types and schemas along with the data, meeting the requirement for containing information about data types.
`
`                        However, lower performance and efficiency for analytical queries.
`                        Additionally, when it comes to file size, worst compression
