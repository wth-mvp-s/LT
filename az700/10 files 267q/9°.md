...
`not interupt connection
`
`1. VM (personal choice)as bridge. so first , to explain the order , I put the VM, as a bridge , (2)on top I make DNS installation. and forwarding rules. 
`
`(3)later onprem configuratiobn so it work together with virtual machine 
`and later last step (4)private endpoint configuration is needed 
