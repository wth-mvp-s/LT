...
`    useful for monitoring and diagnostics of the SQL data warehouse
`    nodes, it doesn't provide specific information about data skew 
`    in a table. 
...
- [x] Workspace: Azure Log Analytics
...
`::
`using integer keys for dates and corresponding integer fields for date references in the fact table is a common technique for optimizing query performance in star schema designs. This approach allows for efficient joins between fact and dimension tables and supports fast querying for arbitrary date ranges and aggregating by fiscal calendar attributes.
...
`What should you recommend?
`::
 `Azure Stream Analytics is the right choice because it's specifically designed to handle real-time data streams, which fits perfectly with the need to monitor IoT devices in real-time. The other services are more oriented towards data integration, transformation, and analysis, but not specifically for real-time streaming data.
...
`
`---------_______☻_|Set variable----|~☻~~~~~~|Stored procedure-|
`--------/---------|(X)Set varialbe1|--------|Stored procedure1|
`-------/
`|Web-|/
`|Web1|\
`-------\-----FAIL--|Set variable----|
`--------\____FAIL__|(X)Set variable2|
`
`Use the drop-down menus to select the answer choice that completes each statement based on the information presented in the graphic.
`::
`The second question is "failed" because web1 for a fail set var2
