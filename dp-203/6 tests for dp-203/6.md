...
`What should you run?
::
`sys.pdw_nodes_column_store_row_groups is a distributed version of the sys.dm_db_column_store_row_group_physical_stats view in SQL Server and provides visibility into the row groups of the columnstore index in an Azure Synapse Analytics dedicated SQL pool. This view includes the deleted_rows column, which indicates the number of deleted rows in each row group.
...
`
`Connect to Pool1 and run DBCC CHECKALLOC: DBCC CHECKALLOC is a command used in traditional SQL Server environments to check the allocation and integrity of database objects. However, it is not the appropriate command to identify data skew in Azure Synapse Analytics dedicated SQL pool.
...
`What should you do?
`::
`The Azure Integration Runtime does not control the compute resources for the Copy Activity, so scaling up the data flow runtime of the Azure Integration Runtime would not impact the Copy Activity's performance.
`
`The Copy Activity's performance depends on the integration runtime that is linked to the source dataset and sink dataset in the Copy Activity. In this case, you have a self-hosted integration runtime for the source and Azure integration runtime for the sink.
`
`You can increase the data movement units (DMUs) in the Copy Activity for Azure Data Factory to maximize the amount of compute resources available to it. However, this is not an option in your question.

- [x] Scale out the self-hosted integration runtime.
