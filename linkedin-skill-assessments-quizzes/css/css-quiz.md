...
#### Q35. When using flexbox, the "justify-content" property can be used to distribute the space between the flex items along the main axis. Which value should be used to evenly distribute the flex items within the container shown below? [[1] [2] [3] [4]] ?? 
...
#### Q101. When the pseudo class selectors are applied to a link, what states will the styles be applied to ? ?? 
...
`:focus styles are applied when a Tab key on a kayboards is used to navigate through links. 
...
`:hover styles are applied on mouse over. 
`:focus styles are applied on mouse click and hold, or when a Tab key on a kayboards is used to navigate through links. 
...
#### Q102. Which element(s) will be displayed in blue text ? ?? 
